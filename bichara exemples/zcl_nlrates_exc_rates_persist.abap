CLASS zcl_nlrates_exc_rates_persist DEFINITION PUBLIC FINAL CREATE PUBLIC.
  PUBLIC SECTION.
    METHODS:
      save_exchange_rates IMPORTING exchange_rates_list TYPE ztnlrates_exchange_rates
                          RETURNING VALUE(result)       TYPE cl_exchange_rates=>ty_messages.

  PRIVATE SECTION.
    METHODS:
      parse_exchange_structure IMPORTING exchange_rates_list TYPE ztnlrates_exchange_rates
                               RETURNING VALUE(result)       TYPE cl_exchange_rates=>ty_exchange_rates.
ENDCLASS.



CLASS zcl_nlrates_exc_rates_persist IMPLEMENTATION.
